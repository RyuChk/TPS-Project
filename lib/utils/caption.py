...
class Caption(object):
    def __init__(
        ...
        device = text.device if isinstance(
            text, torch.Tensor) else torch.device("cpu")
        if isinstance(text, list):
            ...
            if length is None:
                length = torch.stack(
                    [
                        torch.tensor(
                            line.size(0), dtype=torch.int64, device=device)
                        for line in text
                    ]
                )
            ...
            if length is None:
                length = torch.tensor(
                    text.size(-1), dtype=torch.int64, device=device)
            if max_length is None:
